...
class MSSQLTest extends TestCase
{
    ...
    public function testSelectLimit()
    {
        $limit = $this->getQuery()
                ...
                ->setLimit(1)
                ->getSql();
        $this->assertEquals('SELECT * FROM "user" OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY', $this->getTrimmed($limit));

        $offsetLimit = $this->getQuery()
                ->setTable('user')
                ->setLimit(10, 100)
                ->getSql();
        $this->assertEquals('SELECT * FROM "user" OFFSET 100 ROWS FETCH NEXT 10 ROWS ONLY', $this->getTrimmed($offsetLimit));
    }
    ...
    public function testUpdateWithJoin()
    {
        ...
        $this->assertEquals('UPDATE "hostel"'
                . ' SET "hostel"."totalRooms" = COUNT("room"."roomId")'
                . ' FROM "hostel"'
                . ' LEFT JOIN "room" ON "room"."hostelId" = "hostel"."hostelId"'
                . ' WHERE "hostel"."hostelId" = 3', $this->getTrimmed($update));
    }
