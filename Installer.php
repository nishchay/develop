...
/**
 * Installer class.
 * Once application installed, this file gets removed.
 *
...
class Installer
{
    ...
    public static function install($event)
    {
        ...
        $appName = $io->ask('Your application name:') ?? 'My App';

        # Asking user for author name
        $authorName = $io->ask('Your name:') ?? 'Organization';

        # Fetchig application settings content and updating it.
        $applicationContents = str_replace(['{APP_NAME}', '{AUTHOR_NAME}'], [$appName, $authorName], self::getApplicationContents());
        self::updateApplicationContents($applicationContents);
        ...
        $encryptionContents = str_replace(['{PUT_YOUR_MAIN_KEY_HERE}', '{PUT_YOUR_DB_KEY_HERE}'], [$mainKey, $dbKey], self::getEncryptionContents());
        self::updateEncryptionContents($encryptionContents);
